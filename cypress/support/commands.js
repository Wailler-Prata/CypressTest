...
Cypress.Commands.add('verifyValueCardsOfSumValues', (listObjectCards)=>{
    cy.wrap(listObjectCards).each((element) => {
        cy.getVisibeElement(element.cssSelector)
         .invoke('text')
         .then((text) => {           
             cy.wrap(functionsDevfinance.convertStringNumberTo(text).float(), {timeout: 0} ).should('eq', element.value)
         }) 
     })
})